"""
Neural Content-Based model for workout recommendations
"""
import os
import logging
import joblib
...
class NeuralContentBased:
    """Neural Content-Based recommendation model"""
    
    def __init__(self, model_path: Optional[str] = None):
        self.model: Optional[MLPRegressor] = None
        self.scaler: Optional[StandardScaler] = None
        self.model_path = model_path or "models/neural_content_based.pkl"
        self.scaler_path = "models/neural_content_based_scaler.pkl"
        self.feature_names: list = []
        
    def _prepare_features(self, user_item_data: pd.DataFrame,
                         user_features: pd.DataFrame,
                         item_features: pd.DataFrame) -> tuple:
        """
        Prepare features: concatenate user and item features
    ...
    def train(self, user_item_data: pd.DataFrame,
        ...
        """
        logger.info(f"Training Neural Content-Based model on {len(user_item_data)} interactions")
    ...
    def predict(self, user_features: Dict, item_features: Dict) -> float:
        ...
        for feat_name in self.feature_names:
            if '_user' in feat_name:
                base_feat = feat_name.replace('_user', '')
                feature_vector.append(user_features.get(base_feat, 0))
            elif '_item' in feat_name:
                base_feat = feat_name.replace('_item', '')
